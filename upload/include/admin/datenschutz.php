...
if (isset($_POST['sub'])) {
    ...
    if (empty($_POST['id'])) {
        ...
        echo '<div style="width:380px;position:absolute;left:50%;top:31px;margin-left:-190px;display:block;padding:10px;background:#34AF00;padding:5px;color:#000;font-size:12px;font-weight: bold;text-align:center;border:1px solid #000">Der neue Abschnitt wurde eingestellt.</div>';
    } else {
        ...
        echo '<div style="width:380px;position:absolute;left:50%;top:31px;margin-left:-190px;display:block;padding:10px;background:#34AF00;padding:5px;color:#000;font-size:12px;font-weight: bold;text-align:center;border:1px solid #000">Der Abschnitt wurde ge&auml;ndert.</div>';
    }
}
// verschieben
...
while ($row = db_fetch_assoc($erg)) {
    ...
    if ($row['status'] = ($row['einaus'] > '0')) {
        $row['status'] = '<td style="background-color:#2A8E00;color:#fff;text-align:center" title="Abschnitt wird angezeigt"><b>ON</b></td>';
    } else {
        $row['status'] = '<td style="background-color:#ff0000;color:#000;text-align:center" title="Abschnitt wird nicht angezeigt"><b>OFF</b></td>';
    }
    $tpl->set_ar($row);
    $tpl->out(1);
}
